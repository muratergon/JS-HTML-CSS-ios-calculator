  ...
  if (e.target.classList.contains("buttons")) {
    ...
    if (e.target.classList.contains("ac")) {
      ...
      if (
        ...
        secondScreen.innerText.match(/\./) == null
      ) {
      ...
      if (
        ...
        secondScreen.innerText.match(/\./) == null &&
        secondScreen.innerText.length <= 5
      ) {
        secondScreen.innerText += e.target.innerText;
      }
    } else if (e.target.classList.contains("operator")) {
      if (
        firstScreen.innerText.length == 0 &&
        secondScreen.innerText &&
        secondScreen.innerText.match(/[\.]$/) == null
      ) {
        secondScreen.innerText += e.target.innerText;
        ...
        firstScreen.innerText = firstScreen.innerText.replace(
          firstScreen.innerText[firstScreen.innerText.length - 1],
          e.target.innerText
        );
      } else if (
        firstScreen.innerText &&
        secondScreen.innerText &&
        secondScreen.innerText.endsWith(".") == false
      ) {
        operationSecond();
